...
@interface XMNChatController () <XMChatBarDelegate,XMNAVAudioPlayerDelegate,XMNChatMessageCellDelegate,XMNChatViewModelDelegate,UIGestureRecognizerDelegate>
...
- (void)chatBarFrameDidChange:(XMChatBar *)chatBar frame:(CGRect)frame{
    ...
    [UIView animateWithDuration:.3f animations:^{
        [self.tableView setFrame:CGRectMake(0, 0, kscreenWidth, frame.origin.y)];
    } completion:nil];
}
...
- (XMChatBar *)chatBar {
    if (!_chatBar) {
        _chatBar = [[XMChatBar alloc] initWithFrame:CGRectMake(0, kscreenHeight-kMinHeight-64, kscreenWidth, kMinHeight)];
...

{
    ...
    if (_chatBar != nil && point.y < _chatBar.frame.origin.y) {
        [_chatBar endInputing];
    }
