...
- (void)textViewDidChange:(UITextView *)textView{
    ...
    addBarFrame.origin.y = kscreenHeight - self.bottomHeight - addBarFrame.size.height;
...
- (void)faceViewSendFace:(NSString *)faceName{
    if ([faceName isEqualToString:@"[删除]"]) {
        ...
        self.inputText = @"";
        self.textView.text = @"";
        [self setFrame:CGRectMake(0, kscreenHeight - self.bottomHeight - kMinHeight, self.frame.size.width, kMinHeight) animated:NO];
...
- (void)showViewWithType:(XMFunctionViewShowType)showType{
    ...
    switch (showType) {
        ...
        case XMFunctionViewShowVoice:
        {
            ...
            [self setFrame:CGRectMake(0, kscreenHeight - kMinHeight, kscreenWidth, kMinHeight) animated:NO];
        ...
        case XMFunctionViewShowFace:
            self.inputText = self.textView.text;
            [self setFrame:CGRectMake(0, kscreenHeight - kFunctionViewHeight - kMinHeight, kscreenWidth, kFunctionViewHeight) animated:NO];
            [self.textView resignFirstResponder];
            NSLog(@"face show====%f",kscreenHeight);
            [self textViewDidChange:self.textView];
            break;
...
- (void)showFaceView:(BOOL)show{
    if (show) {
        ...
        [UIView animateWithDuration:.3 animations:^{
            [self.faceView setFrame:CGRectMake(0, kscreenHeight - kFunctionViewHeight, self.frame.size.width, kFunctionViewHeight)];
        } completion:nil];
    }else{
        [UIView animateWithDuration:.3 animations:^{
            [self.faceView setFrame:CGRectMake(0, kscreenHeight, self.frame.size.width, kFunctionViewHeight)];
...
- (void)showMoreView:(BOOL)show{
    if (show) {
        ...
        [UIView animateWithDuration:.3 animations:^{
            [self.moreView setFrame:CGRectMake(0, kscreenHeight - kFunctionViewHeight, self.frame.size.width, kFunctionViewHeight)];
        } completion:nil];
    }else{
        [UIView animateWithDuration:.3 animations:^{
            [self.moreView setFrame:CGRectMake(0, kscreenHeight, self.frame.size.width, kFunctionViewHeight)];
        } completion:^(BOOL finished) {
...
- (XMChatFaceView *)faceView{
    if (!_faceView) {
        _faceView = [[XMChatFaceView alloc] initWithFrame:CGRectMake(0, kscreenHeight , self.frame.size.width, kFunctionViewHeight)];
...
- (XMChatMoreView *)moreView{
    if (!_moreView) {
        _moreView = [[XMChatMoreView alloc] initWithFrame:CGRectMake(0, kscreenHeight , self.frame.size.width, kFunctionViewHeight)];
...
- (UIButton *)voiceButton{
    if (!_voiceButton) {
        ...
        [_voiceButton setBackgroundImage:[UIImage imageNamed:@"icon_send_voices"] forState:UIControlStateNormal];
        [_voiceButton setBackgroundImage:[UIImage imageNamed:@"icon_send_voices"] forState:UIControlStateSelected];
...
- (UIButton *)moreButton{
    if (!_moreButton) {
        ...
        [_moreButton setBackgroundImage:[UIImage imageNamed:@"icon_add"] forState:UIControlStateNormal];
        [_moreButton setBackgroundImage:[UIImage imageNamed:@"icon_add"] forState:UIControlStateSelected];
...
- (UIButton *)faceButton{
    if (!_faceButton) {
        ...
        [_faceButton setBackgroundImage:[UIImage imageNamed:@"icon_expression"] forState:UIControlStateNormal];
        [_faceButton setBackgroundImage:[UIImage imageNamed:@"icon_expression"] forState:UIControlStateSelected];
...
- (CGFloat)bottomHeight{
    ...
    if (self.faceView.superview || self.moreView.superview) {
        return MAX(self.keyboardFrame.size.height+height, MAX(self.faceView.frame.size.height, self.moreView.frame.size.height));
    }else{
        return MAX(self.keyboardFrame.size.height+height, CGFLOAT_MIN);
    }
...
- (void)setFrame:(CGRect)frame animated:(BOOL)animated{
    if (frame.origin.y == (kscreenHeight-50)) {
        frame.origin.y -= 64;
    }
