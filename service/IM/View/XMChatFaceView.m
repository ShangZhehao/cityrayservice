...
- (void)changeFaceType:(UIButton *)button{
    ...
    if (self.faceViewType == XMShowRecentFace) {
...
- (SwipeView *)swipeView {
    if (!_swipeView) {
        _swipeView = [[SwipeView alloc] initWithFrame:CGRectMake(0, 10, kscreenWidth, self.frame.size.height - 60)];
...
- (UIPageControl *)pageControl{
    if (!_pageControl) {
        _pageControl = [[UIPageControl alloc] initWithFrame:CGRectMake(0, 120, kscreenWidth, 20)];
...
- (UIView *)bottomView{
    if (!_bottomView) {
        _bottomView = [[UIView alloc] initWithFrame:CGRectMake(0,140,kscreenWidth , 40)];
